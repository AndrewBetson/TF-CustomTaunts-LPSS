...
{
	name		= "[TF2] Custom Taunts - LPSS Version",
...
{
	GameData gameData = new GameData("customtaunts.games");
	if(gameData == INVALID_HANDLE)
		SetFailState("Failed to load gamedata/customtaunts.games.txt.");
...
public Action CommandMenu(int client, int args)
{
	...
	if( args > 1 && !CheckCommandAccess( client, "customtaunts_targetting", ADMFLAG_BAN ) )
	{
